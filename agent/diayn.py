...
def diayn(env_fn,
    ...
    # skill discriminator
    q_phi = core.DIAYNDisc(env.observation_space,
                           num_skills,
                           **ac_kwargs)
    ...
    for epoch in range(1, epochs + 1):
        for t in range(local_steps_per_epoch):
            ...
            q_dist = q_phi._distribution()
            logp_s = q_phi._log_prob_from_distribution(q_dist, s)

            r = ac.pi.entropy * alpha + logp_s # - entropy of uniform

            ep_ret += r
            ep_len += 1
            o = np.concatenate(o, np,array([s]))
